...
- ✅ **CSVインポート機能**（JobsCampaigns形式対応）
  ...
  - クリック率（CTR） → ctr
- ✅ **画像ID入力時の自動フォーム入力**
  - CSVインポート後、画像IDを入力すると該当データが自動入力される
- ✅ **必須フィールドの明確化**
  - 雇用形態（手動入力必須）
  - バナー画像（手動アップロード必須）
...
#### **CSVインポートワークフロー（推奨）**
1. **CSVファイルをアップロード**（JobsCampaigns形式）
   - 対応列：参照番号、企業名、求人、都道府県、表示回数、クリック数、クリック率（CTR）
   - エリア名（「東京都」など）は自動的に辞書コードに変換
2. **画像IDを入力**すると、CSVデータが自動的にフォームに入力される
3. **バナー画像をアップロード**（必須）
   - ドラッグ&ドロップまたはクリックで選択
4. **雇用形態を手動選択**（必須）
   - CSVには含まれないため必ず選択
5. **[AIでタグを提案]ボタン**をクリック（オプション）
   - 画像からOCRでテキストを自動抽出
